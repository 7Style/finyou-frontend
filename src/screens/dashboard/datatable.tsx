    ...
    const [columnDefs, setColumnDefs] = useState<(ColDef | ColGroupDef)[]>([
        {
            ...
            cellRenderer: (props: RenderCellProps) => (
                <div className="flex items-center gap-2">
                    <img src={props.data.image} width={32} height={32} className='rounded-full' alt={props.data.grantName} />
                    <div>
                ...
                <div>
                    <div className="text-xs font-medium text-gray-900 leading-5">{props.data.description}</div>
                    {
                        (props?.data?.progress > 0) && (
                            <ProgressBar containerClassName='dark:bg-amber-700' progressBarClassName="bg-amber-400" progress={props.data.progress} />
                        )
                    }
                </div>
            )
    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            {visibleTags.map((tag, index) => (
                <Badge className="bg-gray-100 text-gray-800 font-medium hover:bg-sky-100 hover:text-blue-800" key={index}>{tag}</Badge>
            ))}
...
// export default function DataTable() {
//     return <ScrollArea className="w-full whitespace-nowrap rounded-md">
