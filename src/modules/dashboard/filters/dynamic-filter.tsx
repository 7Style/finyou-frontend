...
import QueryFilter from '@/modules/dashboard/filters/query-filter'
          ...
          <Command>
            ...
            <CommandGroup>
              {options.map((option, index) => (
                <CommandItem key={index} value={option.name} onSelect={() => handleFilterSelect(option.name)}>
                  {option.label}
                  <IoMdCheckmark
                    className={cn(
                      'ml-auto h-4 w-4',
        ...
        <>
          <QueryBuilderDnD dnd={{ ...ReactDnD, ...ReactDndHtml5Backend }}>
            {selectedOptions.map((option, index) => (
              <QueryFilter key={index} selectedItem={option} fields={options} query={query} onQueryChange={setQuery} />
            ))}
          </QueryBuilderDnD>
          ...
          <Button
            variant={'ghost'}
            size={'sm'}
            className="text-cyan-600 inline-flex items-center font-medium"
