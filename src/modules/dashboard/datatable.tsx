  ...
  const [columnDefs, setColumnDefs] = useState<(ColDef | ColGroupDef)[]>([
    {
      ...
      cellRenderer: (props: RenderCellProps) => (
        <div className="flex items-center gap-2">
          <Image src={props.data.image} width={32} height={32} className="rounded-full" alt={props.data.grantName} />
          <div>
            <div className="text-xs font-medium text-gray-900">{props.data.grantName}</div>
            <div className="text-xs text-gray-500">{props.data.twitterHandle}</div>
          </div>
        </div>
      ),
    },
    {
      ...
      cellRenderer: (props: RenderCellProps) => (
        <div className="flex items-center justify-center gap-4 w-full">
          <button className="text-xs font-medium text-gray-900 cursor-auto">{props.data.companyName}</button>
          <FaEdit
            className={`${isPremiumUser ? 'text-gray-900 cursor-pointer' : 'text-gray-400 cursor-not-allowed'}`}
          />
        </div>
      ),
    },
    {
      ...
      minWidth: 130,
      cellRenderer: (props: RenderCellProps) => (
        <Badge className="bg-green-100 text-green-800 font-medium hover:bg-green-200">{props.data.probability}</Badge>
      ),
    },
    {
      field: 'description',
      ...
      cellRenderer: (props: RenderCellProps) => (
        <div>
          <div className="text-xs font-medium text-gray-900 leading-5">{props.data.description}</div>
    ...
    },
    {
      ...
      flex: 1,
      autoHeight: true,
      cellRenderer: (props: RenderCellProps) => <TagsCell tags={props.data.tags} />,
    },
    {
      field: 'action',
      headerName: 'Action',
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {visibleTags.map((tag, index) => (
        <Badge className="bg-gray-100 text-gray-800 font-medium hover:bg-sky-100 hover:text-blue-800" key={index}>
          {tag}
        </Badge>
      ))}
      {tags.length > initialDisplayCount && <ShowMore count={isExpanded ? 0 : hiddenCount} onClick={toggleTags} />}
    </div>
  )
}
