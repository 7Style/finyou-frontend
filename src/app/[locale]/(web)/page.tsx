  ...
  const projects: ICard[] = [
    {
      ...
      alt: 'marketing',
      title: t('servicePartners'),
      description: t('designMarketingSuppliers'),
    },
    {
      src: '/images/partner.svg',
      alt: 'partner',
      title: t('fePartners'),
      description: t('universityResearchCenters'),
    },
    {
    ...
    <>
        ...
        <Box>
          ...
          <div className="flex items-center flex-wrap gap-4 mb-5">
            {projects.slice(0, 4).map((item, index) => (
              <InlineImageCard key={index} src={item.src} alt={item.alt} title={item.title} />
            ))}
          </div>
      ...
      <Box section>
        <Box>
          ...
          <div className="flex items-center flex-wrap gap-4 mb-5">
            {projects.slice(4, 6).map((item, index) => (
              <InlineImageCard key={index} src={item.src} alt={item.alt} title={item.title} />
            ))}
          </div>
          <div className="flex items-center gap-3 py-2">
        ...
        <Animated>
          <section className="grid md:grid-cols-2 lg:grid-cols-3 py-16 px-4 md:px-12 max-w-screen-2xl mx-auto gap-4">
            {imgCard.map((card, index) => (
              <ImageCard key={index} src={card.src} alt={card.alt} title={card.title} description={card.description} />
            ))}
          </section>
        </Animated>
      </div>
